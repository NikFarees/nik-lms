...
export function CourseStructure({ data }: iAppProps) {
    ...
    // to repopulate the state with the new data
    // to keep the client-side state in sync with the server-side state
    // this is useful when the data changes on the server
    useEffect(() => {
        setItems((prevItems) => {
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        if (activeType === 'chapter') {
            ...
            if (courseId) {
                ...
                toast.promise(reorderChaptersPromise(), {
                    ...
                    success: (result) => {
                        if (result.status === 'success') return 'Chapters reordered successfully';
        ...
        if (activeType === 'lesson' && overType === 'lesson') {
            ...
            if (chapterIndex === -1) {
                toast.error("Could not find the chapter for the lesson");
                return;
            }
            ...
            if (courseId) {
                ...
                toast.promise(reorderLessonsPromise(), {
                    ...
                    success: (result) => {
                        if (result.status === 'success') return 'Lessons reordered successfully';
                        throw new Error(result.message);
                    },
                    error: () => {
                        setItems(previousItems);
